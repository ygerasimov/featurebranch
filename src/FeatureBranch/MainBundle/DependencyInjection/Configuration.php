        ...
        $rootNode
            ->children()
                ...
                ->scalarNode('mysql_root_pass')
                    ...
                    ->example('root')
                ->end()
                ->scalarNode('sniffer_extensions')
                    ->defaultValue('module,inc,php')
                    ->info('Extentions to scan for PHP CodeSniffer')
                    ->example('module,inc,php')
                ->end()
                ->scalarNode('sniffer_ignore_patterns')
                    ->defaultValue('contrib,*.features.*')
                    ->info('Ignored patterns for PHP CodeSniffer')
                    ->example('contrib,*.features.*')
                ->end()
                ->scalarNode('sniffer_standard')
                    ->defaultValue('Drupal')
                    ->info('Sniffing standard to scan with for PHP CodeSniffer')
                    ->example('Drupal')
                ->end()
                ->scalarNode('sniffer_report_file')
                    ->defaultValue('report.txt')
                    ->info('Filename for saving results for PHP CodeSniffer')
                    ->example('report.txt')
                ->end()
            ->end()
        ;
